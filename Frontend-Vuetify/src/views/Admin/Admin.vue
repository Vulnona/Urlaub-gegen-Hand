      ...
      <div class="offers_request_content">
        <div class="card">
          <div class="card-header">
            ...
            <div>
              <!-- Generate Button -->
              <button v-if="!couponCode" class="btn btn-primary" @click="generateCode()">Generate Coupon Code</button>

              <!-- Display the Coupon Code if Available -->
              <p v-if="couponCode">
                <input type="text" :value="couponCode" class="coupon-input" disabled />&nbsp;
                <!-- Copy Button -->
                <!-- <button class="" title="Copy" @click="copyCode()"><i class="ri-clipboard-line"></i></button>&nbsp; -->
                <!-- Clear Button -->
                <button class="" title="Clear" @click="clearCode()"><i class="ri-close-circle-line"></i></button>
              </p>
            </div>
          </div>
          <div class="card-body">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async generateCode() {
        try {
          const res = await axiosInstance.post(`${process.env.baseURL}admin/add-coupon`);
      ...
      async sendCouponCode(uid, code) {
        try {
          const res = await axiosInstance.post(
            `${process.env.baseURL}admin/send-coupon`,
            {
              userId: uid,
              couponCode: code,
            }
