...
export default {
  ...
  data() {
    return {
      ...
      codesStored: false,
      errorMessage: '',
      // Rollback data for safe 2FA reset
      oldTwoFactorEnabled: false,
      oldTwoFactorSecret: null,
      oldBackupCodes: null,
      setupTimeout: null
    };
  },
  mounted() {
    ...
    // Set a timeout for 2FA setup (15 minutes)
    this.setupTimeout = setTimeout(() => {
      if (this.currentStep !== 3) {
        this.errorMessage = '2FA-Setup-Timeout. Bitte starten Sie den Prozess erneut.';
        this.$emit('setup-timeout');
      }
  ...
  methods: {
    async generateQRCode() {
      ...
      try {
        const endpoint = this.isReset ? 'authenticate/2fa/reset' : 'authenticate/2fa/setup';
        const response = await axiosInstance.post(endpoint, { email: this.email });
        
        this.qrCodeData = response.data.qrCodeUri || '';
        this.secret = response.data.secret || '';
        this.backupCodes = response.data.backupCodes || [];
        
        // Store rollback data for reset scenarios
        if (this.isReset && response.data.oldTwoFactorEnabled !== undefined) {
          this.oldTwoFactorEnabled = response.data.oldTwoFactorEnabled;
          this.oldTwoFactorSecret = response.data.oldTwoFactorSecret;
          this.oldBackupCodes = response.data.oldBackupCodes;
        }
        
        toast.success('QR-Code generiert!');
      } catch (err) {
        this.errorMessage = 'Fehler beim Generieren des QR-Codes. Bitte versuchen Sie es erneut.';
        console.error('QR-Code Generierung Fehler:', err);
    ...
    async verifySetup() {
      ...
      try {
        ...
        // Include rollback data for reset scenarios
        if (this.isReset) {
          requestData.oldTwoFactorEnabled = this.oldTwoFactorEnabled;
          requestData.oldTwoFactorSecret = this.oldTwoFactorSecret;
          requestData.oldBackupCodes = this.oldBackupCodes;
        }
        
        const response = await axiosInstance.post('authenticate/2fa/verify-setup', requestData);
        ...
        console.error('2FA Verifizierung Fehler:', err);
