...
namespace UGHApi.Services
{
    public class ReviewService : IReviewService
    {
        ...
        public async Task<string> AddReviewAsync(CreateReviewRequest reviewDto, string email, int? specifiedReviewedId = null)
        {
            ...
            bool isReviewerHost = reviewer.User_Id == offer.User_Id;
            int reviewedId;
            ...
            if (isReviewerHost)
            {
                ...
                var approvedApplication = await _repository.GetApprovedApplicationAsync(reviewDto.OfferId, specifiedReviewedId.Value);
            ...
            {
                var approvedApplication = await _repository.GetApprovedApplicationAsync(reviewDto.OfferId, reviewer.User_Id);
                if (approvedApplication == null)
                {
                    return "User does not have an approved application for this offer.";
                }
                reviewedId = offer.User_Id;
            }

            var existingReview = await _repository.GetReviewByOfferAndUserAsync(reviewDto.OfferId, reviewer.User_Id);
        ...
        public async Task<string> UpdateReviewAsync(int reviewId, CreateReviewRequest reviewDto, string email)
        {
